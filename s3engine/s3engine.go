...
func (s3e *S3Engine) StartUploadWorkers() {
	...
	for i := 0; i < s3e.Concurrency; i++ {
		...
		go s3e.UploadWorker(s3e.JobChannels[i])
	}
}

func (s3e *S3Engine) QueueUploadJob(job *AudioFileJob) {
	s3e.JobChannels[taskCounter%len(s3e.JobChannels)] <- job
}

func (s3e *S3Engine) StopUploadWorkers() {
	for _, ch := range s3e.JobChannels {
		close(ch)
	}
}

func (s3e *S3Engine) UploadWorker(JobChannels chan *AudioFileJob) {
	...
	for {
		...
		case f := <-flushTimeout:
			go s3e.UploadWorker(JobChannels)
